...
const REVIEW_KEY='wmq_review_posts';
const storedReviewPosts=localStorage.getItem(REVIEW_KEY);
let usingDemoReview=storedReviewPosts===null;
let reviewPosts;
try{const parsed=storedReviewPosts===null?defaultReviewPosts:JSON.parse(storedReviewPosts);reviewPosts=Array.isArray(parsed)?parsed:[]}catch(error){reviewPosts=[];usingDemoReview=false}
if(!usingDemoReview)reviewPosts=reviewPosts.map((post,index)=>{const fallback=`legacy-review-${index}-${post.date||''}`;return {...post,id:post.id??fallback,clientId:String(post.clientId||post.id||fallback),sourceTopic:post.sourceTopic||'',contentFormat:post.contentFormat||'',experimentVariable:post.experimentVariable||'',commentInsights:post.commentInsights||'',createdBy:post.createdBy||'当前设备'}});
let activeReviewFilter='all';
let reviewCloudSyncing=false;
let reviewCloudLastSync='';
...
function reviewContentSuggestions(rows,platform,diagnosis){
  if(!rows.length)return [{index:'01',title:'先录入一条已发布内容',detail:'除了后台数据，也填写来源选题、呈现形式和评论区问题，建议会更具体。',tag:'开始积累'}];
  const best=rows.reduce((top,post)=>reviewRowMetrics(post).engagement>reviewRowMetrics(top).engagement?post:top,rows[0]);
  const topic=best.sourceTopic?.trim()||best.title;
  const commentPost=rows.find(post=>post.commentInsights?.trim());
  const commentClue=commentPost?.commentInsights?.trim().split(/[。！？\n；;]/).find(Boolean)?.trim();
  const targetPlatform=best.platform==='xhs'?'抖音':'小红书';
  const adaptation=best.platform==='xhs'
    ?`把高互动内容「${best.title}」改成 45—60 秒口播：先给结论，再讲 3 个具体动作。`
    :`把高播放内容「${best.title}」整理成可收藏的图文清单，标题保留同一人群与场景。`;
  return [
    {index:'01',title:`把「${topic}」做成第 2 集`,detail:`延续已经被验证的核心问题，补充“最容易踩坑的 3 个细节”或一个更具体的职场场景。`,tag:'系列延伸'},
    {index:'02',title:diagnosis.experiment,detail:`保持选题、呈现形式和发布时间尽量不变。${diagnosis.detail}`,tag:`单变量实验 · ${diagnosis.weak.label}`},
    commentClue
      ?{index:'03',title:`回应评论区：“${commentClue.slice(0,42)}${commentClue.length>42?'…':''}”`,detail:'把用户真实提问直接放在开头，先回答结论，再补充适用边界和一个可复制的话术。',tag:'评论区新选题'}
      :{index:'03',title:`改编到${targetPlatform}再验证一次`,detail:adaptation,tag:'跨平台复用'}
  ];
}
function renderReviewSuggestions(rows,platform,diagnosis){
  const suggestions=reviewContentSuggestions(rows,platform,diagnosis);
  $('#reviewSuggestionList').innerHTML=suggestions.map(item=>`<article class="review-suggestion"><span>${item.index}</span><div><b>${escapeHtml(item.title)}</b><p>${escapeHtml(item.detail)}</p><em>${escapeHtml(item.tag)}</em></div></article>`).join('');
}
function reviewCloudConfigured(){return Boolean(window.ReviewCloud?.isConfigured())}
function cloudRowToReview(row){return {id:row.id,cloudId:row.id,cloudWorkspaceId:row.workspace_id,clientId:String(row.client_id),platform:row.platform,date:row.published_at,title:row.title,sourceTopic:row.source_topic||'',contentFormat:row.content_format||'',experimentVariable:row.experiment_variable||'',commentInsights:row.comment_insights||'',impressions:Number(row.impressions)||0,views:Number(row.views)||0,completion:Number(row.completion)||0,hook5:Number(row.hook5)||0,likes:Number(row.likes)||0,comments:Number(row.comments)||0,saves:Number(row.saves)||0,shares:Number(row.shares)||0,follows:Number(row.follows)||0,avgWatch:Number(row.avg_watch)||0,createdBy:row.created_by||'团队成员',updatedAt:row.updated_at||'',cloudPending:false}}
function reviewToCloudRow(post){const config=window.ReviewCloud.getConfig();return {workspace_id:config.workspaceId,client_id:String(post.clientId||post.id),platform:post.platform,published_at:post.date,title:post.title,source_topic:post.sourceTopic||'',content_format:post.contentFormat||'',experiment_variable:post.experimentVariable||'',comment_insights:post.commentInsights||'',impressions:Number(post.impressions)||0,views:Number(post.views)||0,completion:Number(post.completion)||0,hook5:Number(post.hook5)||0,likes:Number(post.likes)||0,comments:Number(post.comments)||0,saves:Number(post.saves)||0,shares:Number(post.shares)||0,follows:Number(post.follows)||0,avg_watch:Number(post.avgWatch)||0,created_by:post.createdBy||config.displayName||'团队成员',updated_at:new Date().toISOString()}}
function updateReviewCloudUi(state='local',detail=''){
  const configured=reviewCloudConfigured(),bar=$('#reviewCloudBar'),config=window.ReviewCloud?.getConfig?.()||{};
  bar.classList.remove('is-connected','is-syncing','is-offline','is-error');
  if(state==='syncing')bar.classList.add('is-syncing');else if(state==='offline')bar.classList.add('is-offline');else if(state==='error')bar.classList.add('is-error');else if(configured)bar.classList.add('is-connected');
  const title={syncing:'正在同步团队数据',offline:'离线保存，等待同步',error:'云端连接需要检查'}[state]||(configured?'团队云端已连接':'当前为本机模式');
  const fallback=configured?(reviewCloudLastSync?`最近同步 ${reviewCloudLastSync} · 团队工作区 ${config.workspaceId}`:`团队工作区 ${config.workspaceId} · 将自动同步`):'连接团队云端后，一人更新，其他成员也能看到';
  $('#reviewCloudTitle').textContent=title;$('#reviewCloudDetail').textContent=detail||fallback;$('#openReviewCloud').textContent=configured?'云端设置':'连接云端';$('#syncReviewCloud').hidden=!configured;$('#syncReviewCloud').disabled=state==='syncing';
  $('#settingsReviewCloudState').textContent=configured?`已连接 ${config.workspaceId} · 复盘数据团队共享`:'尚未连接 · 复盘数据仍在本机';
}
const saveReviewPosts=()=>localStorage.setItem(REVIEW_KEY,JSON.stringify(reviewPosts));
async function syncReviewCloud({quiet=false}={}){
  if(!reviewCloudConfigured()){updateReviewCloudUi('local');return false}
  if(reviewCloudSyncing)return false;
  reviewCloudSyncing=true;updateReviewCloudUi('syncing');
  try{
    if(!usingDemoReview){
      const pending=reviewPosts.filter(post=>post.cloudPending||!post.cloudId);
      if(pending.length)await window.ReviewCloud.upsert(pending.map(reviewToCloudRow));
    }
    const cloudRows=await window.ReviewCloud.list();
    if(cloudRows.length||!usingDemoReview){reviewPosts=cloudRows.map(cloudRowToReview);usingDemoReview=false;saveReviewPosts()}
    reviewCloudLastSync=new Date().toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit'});updateReviewCloudUi('synced');renderReview();renderOverview();
    if(!quiet)showToast('团队复盘数据已同步');
    return true;
  }catch(error){
    const pendingCount=usingDemoReview?0:reviewPosts.filter(post=>post.cloudPending||!post.cloudId).length;
    updateReviewCloudUi(navigator.onLine?'error':'offline',`${error.message||'暂时无法连接云端'}${pendingCount?` · ${pendingCount} 条保留在本机待同步`:''}`);
    if(!quiet)showToast(pendingCount?'已保存在本机，联网后会再同步':'云端同步失败，请检查设置');
    return false;
  }finally{reviewCloudSyncing=false;$('#syncReviewCloud').disabled=false}
}
function renderReview(filter=activeReviewFilter){
  ...
  const pendingCount=usingDemoReview?0:reviewPosts.filter(post=>post.cloudPending||(!post.cloudId&&reviewCloudConfigured())).length;
  $('#reviewDataMode').textContent=usingDemoReview?'当前展示演示样本；录入首条真实数据后会自动清空演示数据。':reviewCloudConfigured()?`正在使用团队共享数据${pendingCount?`；${pendingCount} 条等待上传。`:'，成员更新后会自动出现在这里。'}`:'当前是真实数据，但仍只保存在这台设备；连接云端即可多人共享。';$('#reviewPlatformLabel').textContent=label;$('#reviewSample').textContent=`基于 ${rows.length} 条内容`;$('#reviewCount').textContent=rows.length;$('#reviewConfidence').textContent=rows.length<3?'样本偏少':rows.length<8?'样本可用':'趋势较稳';
  if(!rows.length){const diagnosis=reviewDiagnosis([],filter);$('#reviewVerdict').textContent='还没有可复盘的数据';$('#reviewReason').textContent='录入一条内容或导入 CSV 后，这里会自动生成总结。';$('#reviewEngagement').textContent='0.0';$('#reviewFollow').textContent='0.0';$('#funnelExposureValue').textContent='0%';$('#funnelViewValue').textContent='0%';$('#funnelEngageValue').textContent='0%';$('#funnelFollowValue').textContent='0%';['#funnelExposure','#funnelView','#funnelEngage','#funnelFollow'].forEach(selector=>$(selector).style.width='0%');$('#reviewBestTitle').textContent='等待真实数据';$('#reviewBestReason').textContent='录入后自动识别表现最好的内容。';$('#reviewWeakPoint').textContent='等待样本';$('#reviewWeakReason').textContent='至少积累三条同平台内容，结论会更可靠。';$('#reviewExperiment').textContent='先录入一条已经发布的内容';$('#reviewExperimentDetail').textContent='把平台后台的真实数据填入，系统会自动生成下一步建议。';$('#reviewPostList').innerHTML='<p class="list-empty-state">当前筛选下还没有真实内容数据。</p>';renderReviewSuggestions([],filter,diagnosis);return}
  ...
  $('#reviewVerdict').textContent=diagnosis.verdict;$('#reviewReason').textContent=`「${best.title}」的综合互动率最高（${reviewPercent(bestMetrics.engagement)}%）。${diagnosis.reason}`;$('#reviewBestTitle').textContent=best.title;$('#reviewBestReason').textContent=`综合互动率 ${reviewPercent(bestMetrics.engagement)}%，收藏率 ${reviewPercent(bestMetrics.save)}%。`;$('#reviewWeakPoint').textContent=diagnosis.weak.label;$('#reviewWeakReason').textContent=diagnosis.reason;$('#reviewExperiment').textContent=diagnosis.experiment;$('#reviewExperimentDetail').textContent=diagnosis.detail;renderReviewSuggestions(rows,filter,diagnosis);
  $('#reviewPostList').innerHTML=rows.map(post=>{const metric=reviewRowMetrics(post),pending=post.cloudPending||(!post.cloudId&&reviewCloudConfigured());return `<article class="review-post"><div class="review-post-top"><span class="review-post-platform ${post.platform}">${post.platform==='xhs'?'小红书':'抖音'}</span><div class="review-post-actions"><span class="review-post-date">${escapeHtml(post.date)}${pending?' · 待同步':''}</span>${usingDemoReview?'':`<button class="item-delete-button" data-delete-review="${escapeHtml(post.id)}" aria-label="删除${escapeHtml(post.title)}">删</button>`}</div></div><h3>${escapeHtml(post.title)}</h3>${post.sourceTopic?`<p class="review-post-source">来源选题 · ${escapeHtml(post.sourceTopic)}</p>`:''}<div class="review-post-metrics"><span>阅读/播放<b>${Number(post.views).toLocaleString()}</b></span><span>互动率<b>${reviewPercent(metric.engagement)}%</b></span><span>收藏率<b>${reviewPercent(metric.save)}%</b></span><span>涨粉率<b>${reviewPercent(metric.follow)}%</b></span></div></article>`}).join('');
  $$('#reviewPostList [data-delete-review]').forEach(button=>button.addEventListener('click',async()=>{const post=reviewPosts.find(row=>String(row.id)===button.dataset.deleteReview),confirmed=await askConfirm({title:'删除这条复盘数据？',message:post?`“${post.title}”及其指标会被删除，团队成员也将不再看到。`:'删除后复盘结论会重新计算。'});if(!confirmed)return;if(reviewCloudConfigured()&&post?.cloudId){updateReviewCloudUi('syncing','正在从团队云端删除…');try{await window.ReviewCloud.remove(post.cloudId)}catch(error){updateReviewCloudUi('error',error.message);showToast('云端删除失败，记录已保留');return}}reviewPosts=reviewPosts.filter(row=>String(row.id)!==button.dataset.deleteReview);saveReviewPosts();renderReview();renderOverview();updateReviewCloudUi(reviewCloudConfigured()?'synced':'local');showToast('复盘数据已删除，结论已更新')}));
}
...
$('#reviewForm').addEventListener('submit',async event=>{event.preventDefault();const number=id=>Number($(id).value)||0,platform=$('#reviewPlatform').value,clientId=crypto.randomUUID?.()||`review-${Date.now()}-${Math.random().toString(16).slice(2)}`,config=window.ReviewCloud?.getConfig?.()||{};if(usingDemoReview){reviewPosts=[];usingDemoReview=false}reviewPosts.unshift({id:clientId,clientId,platform,date:$('#reviewDate').value,title:$('#reviewTitle').value.trim(),sourceTopic:$('#reviewSourceTopic').value.trim(),contentFormat:$('#reviewContentFormat').value,experimentVariable:$('#reviewExperimentVariable').value.trim(),commentInsights:$('#reviewCommentInsights').value.trim(),impressions:number('#reviewImpressions')||number('#reviewViews'),views:number('#reviewViews'),completion:number('#reviewCompletion'),hook5:number('#reviewHook'),likes:number('#reviewLikes'),comments:number('#reviewComments'),saves:number('#reviewSaves'),shares:number('#reviewShares'),follows:number('#reviewFollows'),avgWatch:number('#reviewAvgWatch'),createdBy:config.displayName||'当前设备',cloudPending:reviewCloudConfigured()});saveReviewPosts();$('#reviewForm').reset();$('#reviewDialog').close();selectReviewFilter(platform);$('#reviewImportStatus').textContent=reviewCloudConfigured()?'✓ 已保存在本机，正在同步团队云端':'✓ 已保存在本机，并生成后续内容建议';if(reviewCloudConfigured())await syncReviewCloud({quiet:true});$('#reviewImportStatus').textContent=reviewCloudConfigured()?'✓ 已同步，并完成新一轮复盘':'✓ 已保存在本机，并完成新一轮复盘';setTimeout(()=>$('#reviewImportStatus').textContent='',3000)});
...
$('#reviewCsv').addEventListener('change',event=>{const file=event.target.files[0];if(!file)return;const reader=new FileReader();reader.onload=async()=>{try{const rows=parseCsvRows(String(reader.result||'')),headers=(rows.shift()||[]).map(header=>header.trim()),position=name=>headers.indexOf(name),value=(row,name)=>position(name)>=0?(row[position(name)]??''):'',config=window.ReviewCloud?.getConfig?.()||{};let added=0,imported=[];rows.forEach(row=>{let platform=value(row,'platform').toLowerCase();if(platform==='小红书')platform='xhs';if(platform==='抖音')platform='douyin';const title=value(row,'title');if(!['xhs','douyin'].includes(platform)||!title)return;const number=name=>Number(value(row,name))||0,clientId=crypto.randomUUID?.()||`review-${Date.now()}-${added}`;imported.push({id:clientId,clientId,platform,date:value(row,'date')||dateKey(today),title,sourceTopic:value(row,'sourceTopic'),contentFormat:value(row,'contentFormat'),experimentVariable:value(row,'experimentVariable'),commentInsights:value(row,'commentInsights'),impressions:number('impressions')||number('views'),views:number('views'),completion:number('completion'),hook5:number('hook5'),likes:number('likes'),comments:number('comments'),saves:number('saves'),shares:number('shares'),follows:number('follows'),avgWatch:number('avgWatch'),createdBy:config.displayName||'当前设备',cloudPending:reviewCloudConfigured()});added++});if(!added)throw new Error('no rows');if(usingDemoReview){reviewPosts=[];usingDemoReview=false}reviewPosts.push(...imported);saveReviewPosts();selectReviewFilter('all');$('#reviewImportStatus').textContent=`✓ 已导入 ${added} 条内容，正在生成建议`;if(reviewCloudConfigured())await syncReviewCloud({quiet:true});$('#reviewImportStatus').textContent=`✓ 已导入 ${added} 条内容并完成复盘${reviewCloudConfigured()?' · 已同步团队':''}`}catch(error){$('#reviewImportStatus').textContent='未识别到有效数据，请使用页面提供的 CSV 模板'}event.target.value=''};reader.readAsText(file,'UTF-8')});

function openReviewCloudDialog(){const config=window.ReviewCloud?.getConfig?.()||{};$('#reviewCloudDisplayName').value=config.displayName||'';$('#reviewCloudUrl').value=config.supabaseUrl||'';$('#reviewCloudAnonKey').value=config.supabaseAnonKey||'';$('#reviewCloudWorkspaceId').value=config.workspaceId||'wmq-team';$('#reviewCloudWorkspaceKey').value=config.workspaceKey||'';$('#reviewCloudFormStatus').textContent='';$('#reviewCloudFormStatus').classList.remove('is-error');$('#disconnectReviewCloud').hidden=!reviewCloudConfigured();$('#reviewCloudDialog').showModal()}
$('#openReviewCloud').addEventListener('click',openReviewCloudDialog);$('#settingsReviewCloud').addEventListener('click',openReviewCloudDialog);$('#syncReviewCloud').addEventListener('click',()=>syncReviewCloud());
$('#reviewCloudForm').addEventListener('submit',async event=>{event.preventDefault();const button=$('#saveReviewCloud'),previous=window.ReviewCloud.getConfig(),hadPrevious=reviewCloudConfigured(),next={displayName:$('#reviewCloudDisplayName').value,supabaseUrl:$('#reviewCloudUrl').value,supabaseAnonKey:$('#reviewCloudAnonKey').value,workspaceId:$('#reviewCloudWorkspaceId').value,workspaceKey:$('#reviewCloudWorkspaceKey').value};button.disabled=true;$('#reviewCloudFormStatus').classList.remove('is-error');$('#reviewCloudFormStatus').textContent='正在测试团队云端…';window.ReviewCloud.saveConfig(next);try{if(!reviewCloudConfigured())throw new Error('请完整填写项目网址、anon key、工作区和团队密码');await window.ReviewCloud.test();if(!usingDemoReview){const workspaceId=window.ReviewCloud.getConfig().workspaceId;reviewPosts=reviewPosts.map(post=>post.cloudWorkspaceId===workspaceId?post:{...post,id:post.clientId||post.id,cloudId:'',cloudWorkspaceId:'',cloudPending:true});saveReviewPosts()}$('#reviewCloudFormStatus').textContent='✓ 连接成功，正在合并本机与团队数据';await syncReviewCloud({quiet:true});$('#reviewCloudDialog').close();showToast('团队复盘已连接，后续会自动同步')}catch(error){if(hadPrevious)window.ReviewCloud.saveConfig(previous);else window.ReviewCloud.clearConfig();$('#reviewCloudFormStatus').classList.add('is-error');$('#reviewCloudFormStatus').textContent=error.message||'连接失败，请检查填写内容';updateReviewCloudUi(reviewCloudConfigured()?'synced':'local')}finally{button.disabled=false}});
$('#disconnectReviewCloud').addEventListener('click',()=>{window.ReviewCloud.clearConfig();updateReviewCloudUi('local');renderReview();$('#reviewCloudDialog').close();showToast('当前设备已断开；本机复盘数据仍然保留')});
...
$('#exportBackup').addEventListener('click',()=>{
  const backup={exportedAt:new Date().toISOString(),version:21,data:{}};
  for(let index=0;index<localStorage.length;index++){const key=localStorage.key(index);if(key?.startsWith('wmq_')&&key!=='wmq_review_cloud')backup.data[key]=localStorage.getItem(key)}
...
function navigate(name,updateUrl=true){
  ...
  if(name==='review'&&reviewCloudConfigured())syncReviewCloud({quiet:true});
}
...
applySettings();updateReviewCloudUi(reviewCloudConfigured()?'synced':'local');renderCalendar();loadDailyRecord();renderWorkoutProgress();renderFitnessHistory();renderCreationLibrary();renderTrends();updateTrendStatus();loadTrendData();renderTransactions();renderReview();renderOverview();renderJournalHistory();
...
window.addEventListener('focus',()=>{if(reviewCloudConfigured())syncReviewCloud({quiet:true})});
document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'&&reviewCloudConfigured())syncReviewCloud({quiet:true})});
setInterval(()=>{if(document.visibilityState==='visible'&&reviewCloudConfigured())syncReviewCloud({quiet:true})},45000);
if(reviewCloudConfigured())syncReviewCloud({quiet:true});
